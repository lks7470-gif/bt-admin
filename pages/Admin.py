...
<style>
    ...
    @media print {
        ...
        /* 상단 정보 테이블 */
        .info-table { width: 100%; border-collapse: collapse; border: 2px solid black !important; margin-bottom: 10px; font-size: 11pt; }
        .info-table th { background: #f0f0f0 !important; font-weight: bold; width: 18%; border: 1px solid black !important; padding: 5px; }
        .info-table td { text-align: center; border: 1px solid black !important; padding: 5px; }

        /* [핵심] QR 그리드 (3열 x 3행) + 간격 띄우기 */
        .qr-table { 
            width: 100%; 
            border-collapse: separate; /* 칸 분리 필수 */
            border-spacing: 15px; /* 👈 QR 사이 간격을 15px 띄움 (스캔 오류 방지) */
            table-layout: fixed; 
        }
        ...
        .qr-cell { 
            width: 33.33%; 
            height: 60mm; 
            border: 2px solid black !important; /* 각 칸마다 테두리 */
            text-align: center; 
            vertical-align: middle; 
            padding: 5px;
            border-radius: 10px; /* 둥근 모서리로 구분감 줌 */
        }
        ...
        .txt-dim { font-size: 16pt; font-weight: 900; margin-bottom: 2px; display: block; }
        .txt-elec { font-size: 12pt; font-weight: bold; margin-bottom: 5px; display: block; }
        .txt-lot { font-size: 10pt; font-weight: bold; margin-top: 2px; font-family: monospace; display: block; }
...
<div style="font-size:16pt; font-weight:bold; margin-bottom:10px;">📋 생산 리스트 (총 {len(items)}개)</div>
    ...
    # ------------------------------------
        ...
        if st.session_state.history_data:
            ...
            if not selected_rows.empty:
                ...
                # [중요] 미리보기 HTML을 렌더링해둬야 인쇄 시 나옵니다.
...
with tab9:
    ...
    with c2:
        ...
        if st.button("🖨️ QR 인쇄하기", type="primary", use_container_width=True):
            components.html("<script>parent.window.print()</script>", height=0, width=0)import streamlit as st
import streamlit.components.v1 as components
import pandas as pd
import qrcode
import io
