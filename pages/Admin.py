...
<style>
    ...
    /* 인쇄 전용 스타일 */
    @media print {
        /* 1. 페이지 여백 제거 (브라우저 설정 무시 강제) */
        @page { size: A4 portrait; margin: 0; }
        
        /* 2. 스트림릿 UI 숨김 */
        body * { visibility: hidden; }
        header, footer, .stButton, .stHeader, .stSidebar { display: none !important; }

        /* 3. 인쇄 영역 설정 (Flexbox Layout) */
        .printable-area {
        ...
        /* 테이블 섹션 (남는 공간 모두 차지) */
        .table-section {
            flex: 1 1 auto; /* 남는 공간 모두 가짐 */
            display: flex;
            flex-direction: column;
            border: 2px solid black; /* 외곽 테두리 */
        }
        ...
        .qr-table { 
            width: 100%; 
            height: 100%; /* 부모(table-section) 높이 꽉 채움 */
        ...
        /* QR 이미지 */
        .qr-img { width: 150px; height: 150px; margin: 10px auto; display: block; }
        
        /* 텍스트 사이즈 확대 */
        .txt-dim { font-size: 24pt; font-weight: 900; margin-bottom: 5px; display: block; }
        .txt-elec { font-size: 16pt; font-weight: bold; margin-bottom: 5px; display: block; }
        .txt-lot { font-size: 11pt; font-weight: bold; margin-top: 5px; font-family: monospace; display: block; }
        .txt-info { font-size: 10pt; display: block; }

        .top-time { position: absolute; top: 5mm; right: 10mm; font-size: 9pt; color: #555; }
        .footer-warning { text-align: center; font-size: 10pt; font-weight: bold; margin-top: 5px; }
    }
