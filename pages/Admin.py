...
# ==========================================
# 🔥 [스타일] CSS 정의 (최종: 선 제거 + 숫자 강조 + A4 꽉 채움)
# ==========================================
PRINT_CSS = """
<style>
...
def create_a4_html(header, items):
    ...
    for item in cells_data:
        if item:
            ...
            # [핵심] 전극 정보 내 숫자만 찾아서 강조 (Bold & Size Up)
            elec_str = str(item["elec"])
            # 숫자(\d+)를 찾아서 <span class="num-bold">숫자</span>로 감쌈
            ...
            if not selected_rows.empty:
                ...
                for _, row in selected_rows.iterrows():
                    ...
                    try:
                        ...
                        if match: w, h = match.group(1), match.group(2)
                ...
                # '방금 발행'과 동일한 함수 사용하여 일관된 디자인 적용
