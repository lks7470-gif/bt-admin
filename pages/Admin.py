    ...
    if not edited.empty:
        for i, row in edited.iterrows():
            try:
                ...
                # PDF 단가 기준표 규칙 적용
                if w > 0 and h > 0: [cite: 1, 8]
                    rw = math.ceil(w / 100) * 100 [cite: 8, 9, 10]
                    rh = math.ceil(h / 100) * 100 [cite: 8, 9, 10]
                    area = (rw * rh) / 1_000_000
                    
                    base_price = 300150 [cite: 2]
                    p_w = 0; p_h = 0
                    if rw > 2000: p_w = 0.15 [cite: 13]
                    elif rw > 1800: p_w = 0.10 [cite: 13]
                    elif rw > 1500: p_w = 0.05 [cite: 13]
                    
                    if rh > 3200: p_h = 0.15 [cite: 27, 28]
                    elif rh > 2800: p_h = 0.10 [cite: 23, 24]
                    elif rh > 2400: p_h = 0.05 [cite: 19, 20]
                    elif rh > 2000: p_h = 0.0 [cite: 14, 15, 16]
                    
                    f_glass = 1.12 if "Low" in str(row.get('유리','')) else 1.0 [cite: 4]
                    f_thick = 1.0 [cite: 6]
                    if "5+5" in str(row.get('두께','')): f_thick = 1.08 [cite: 6]
                    elif "6+6" in str(row.get('두께','')): f_thick = 1.15 [cite: 6]
                    
                    unit_m2_price = base_price * f_glass * f_thick * (1 + p_w + p_h) [cite: 2, 4, 6, 9]
