...
# ==============================================================================
# 🛠️ [기능 정의 구역] 함수들을 먼저 정의합니다 (에러 방지)
# ==============================================================================
...
# 8. 작업지시서 A4 2x4 HTML (디자인 및 강조 수정됨)
def get_work_order_html(items):
    ...
    for i in range(0, len(items), chunk_size):
        ...
        for item in chunk:
            qr_clean = item['lot'].replace("-", "") # QR 하이픈 제거
            qr = qrcode.QRCode(box_size=5, border=0); qr.add_data(qr_clean); qr.make(fit=True)
            img_b64 = image_to_base64(qr.make_image(fill_color="black", back_color="white"))
...
def get_quotation_html(cust_data, items_df, totals):
    ...
    for _, row in items_df.iterrows():
        html += f"""<tr class="item-row"><td class="txt-c">{row['구분']}</td><td>{row['품명']}</td><td class="txt-c">{row['수량']}</td><td class="txt-r">{int(row['단가']):,}</td><td class="txt-r">{int(row['공급가']):,}</td><td>{row['비고']}</td></tr>"""
...
with tab1:
    ...
    with st.form("order_form"):
        ...
        fabric_short = c_mat2.text_input("🆔 식별코드 (4자리)", value=default_short, max_chars=4, key=f"short_{fabric_lot}")
        ...
        if not sel.empty:
            if st.button("🖨️ 선택 항목 재발행"):
                ...
                for _, r in sel.iterrows():
                    ...
                    rep_items.append({"lot": r['lot_no'], "cust": r['customer'], "prod": r['product'], "w": w, "h": h, "elec": elec, "fabric": r.get('fabric_lot_no','-'), "spec": r.get('spec',''), "note": r.get('note','')})
...
with tab10:
    ...
    if not edited.empty:
        # 간단 계산
        edited['공급가'] = pd.to_numeric(edited['수량']) * pd.to_numeric(edited['단가'])
        ...
        if st.button("🖨️ 견적서 인쇄"):
            ...
            components.html(generate_print_html(html), height=0)import streamlit as st
import streamlit.components.v1 as components
import pandas as pd
import qrcode
import io
