...
# ----------------------------------------------------
def get_work_order_html(items, header_info=None):
    html = """
    <html>
    <head>
        <style>
    ...
    for item in items:
        ...
        # 스펙 정보가 문자열 하나로 합쳐져 있을 경우 분리 시도 (DB 조회 시)
        ...
        if lam_cond == "해당 없음" or lam_cond == "-": is_lam = False
...
with tab1:
    ...
    if st.session_state.order_list:
        ...
        st.dataframe(pd.DataFrame(st.session_state.order_list)[["고객사", "lot_short", "제품", "규격", "수량"]], use_container_width=True)
        ...
        if c2.button("🚀 최종 발행 및 저장 (Supabase)", type="primary", use_container_width=True):
            ...
            for item in st.session_state.order_list:
                ...
                for _ in range(item['수량']):
                    seq_str = f"{cnt:02d}"
                    # ⭐ [최종 ID] 약어(4)+날짜(6)+제품(1)+순번(2) = 13자리
